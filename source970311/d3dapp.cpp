...
D3DAppSwapTextures()
{
    ...
    for (i = 0; i < d3dappi.NumUsableTextures - 1; i++) {
        ...
        d3dappi.lpD3DDevice->SwapTextureHandles(d3dappi.lpTexture[i],d3dappi.lpTexture[i+1]);
...
D3DAppShowBackBuffer(DWORD flags)
{
    ...
    } else 
	{
		...
		{
            LastError =
                    d3dappi.lpFrontBuffer->Blt(&front[i], d3dappi.lpBackBuffer,
...
D3DAppClearBackBuffer(DWORD flags)
{
    ...
    if (flags & D3DAPP_CLEARALL) {
        ...
        dummy.y2 = d3dappi.szClient.cy;
        LastError =
                  d3dappi.lpD3DViewport->Clear(1,&dummy,clearflags);
        if (LastError != D3D_OK) {
         ...
         */
        LastError = d3dappi.lpD3DViewport->Clear(NumDirtyBackRects,DirtyBack, D3DCLEAR_TARGET);
        if (LastError != D3D_OK) {
         ...
         */
        LastError = d3dappi.lpD3DViewport->Clear(NumDirtyZRects,DirtyZ,D3DCLEAR_ZBUFFER);
        if (LastError != D3D_OK) {
