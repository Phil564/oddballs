...
D3DAppIEnumDisplayModes(void)
{
	...
	{
		dprintf("Res: %d, %dx%d\n", i, d3dappi.Mode[i].w, d3dappi.Mode[i].h);
...
D3DAppICreateSurface(LPDDSURFACEDESC lpDDSurfDesc,
    ...
    result = d3dappi.lpDD->CreateSurface(lpDDSurfDesc,lpDDSurface,NULL);
    return result;
}
...
D3DAppICheckForPalettized(void)
{
    ...
    if (bPrimaryPalettized) {
         ...
         */
        LastError = d3dappi.lpDD->CreatePalette(DDPCAPS_8BIT | DDPCAPS_INITIALIZE,ppe,&lpPalette,NULL);
        if (LastError != DD_OK) {
...
D3DAppICreateZBuffer(int w, int h, int driver)
{
     ...
     */
    LastError =
       d3dappi.lpBackBuffer->AddAttachedSurface(d3dappi.lpZBuffer);
    if(LastError != DD_OK) {
...
D3DAppISetCoopLevel(HWND hwnd, BOOL bFullscreen)
{
    if (bFullscreen) {
        bIgnoreWM_SIZE = TRUE;
        LastError = d3dappi.lpDD->SetCooperativeLevel(hwnd,DDSCL_EXCLUSIVE | DDSCL_FULLSCREEN | DDSCL_ALLOWMODEX);
...
D3DAppIClearBuffers(void)
{
    ...
    if (d3dappi.lpFrontBuffer) {
        ...
        LastError = d3dappi.lpFrontBuffer->Blt(&dst,NULL,NULL,DDBLT_COLORFILL | DDBLT_WAIT,&ddbltfx);
        if (LastError != DD_OK) {
    ...
    if (d3dappi.lpBackBuffer) {
        ...
        ddbltfx.dwSize = sizeof(DDBLTFX);
        SetRect(&dst, 0, 0, ddsd.dwWidth, ddsd.dwHeight);
        LastError = d3dappi.lpBackBuffer->Blt(&dst,NULL,NULL,DDBLT_COLORFILL | DDBLT_WAIT,&ddbltfx);
        if (LastError != DD_OK) {
            D3DAppISetErrorString("Clearing the front buffer failed.\n%s",
                                  D3DAppErrorToString(LastError));
            return FALSE;
