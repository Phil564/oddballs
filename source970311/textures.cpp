...
D3DAppILoadTextureSurf(int n, BOOL* bInVideo)
{
    ...
    LastError = lpSrcTextureSurf->QueryInterface(IID_IDirect3DTexture,(LPVOID*)&lpSrcTexture);
    if (LastError != DD_OK) {
    ...
    if (pcaps) {
        ...
        LastError = d3dappi.lpDD->CreatePalette(pcaps,ppe,&lpDstPalette,NULL);
        if (LastError != DD_OK) {
...
//      lpDstPalette->Release();
     ...
     */
    LastError = d3dappi.lpTextureSurf[n]->QueryInterface(IID_IDirect3DTexture,(LPVOID*)&d3dappi.lpTexture[n]);
    if (LastError != DD_OK) {
                   ...
                   LPDDSURFACEDESC lpFormat, DWORD memoryflag)
{
	...
	if (format.ddpfPixelFormat.dwFlags & DDPF_PALETTEINDEXED4) 
	{
        bQuant = TRUE;
        psize = 16;
        pcaps = DDPCAPS_4BIT;
...
//burst_colors:
     ...
     */
    ddrval = lpDD->CreatePalette(DDPCAPS_INITIALIZE | pcaps,ppe,&lpDDPal,NULL);
    if (ddrval != DD_OK) {
        lpDDS->Release();
