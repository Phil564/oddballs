...
//        if ((pddsd->dwWidth == 512) && (pddsd->dwHeight == 384) && (pddsd->ddpfPixelFormat.dwRGBBitCount == 16))
        if ((pddsd->dwWidth == 640) && ((pddsd->dwHeight == 480)) && (pddsd->ddpfPixelFormat.dwRGBBitCount == 16))
        {
            g_dwCurrMode = g_dwNumModes;
			dprintf("found optimal resolution..");
    ...
    if (g_bSoundPresent)
    {
        ...
        TRY_DS(g_lpDS->SetCooperativeLevel(g_hWnd, DSSCL_NORMAL))
            ...
            return FALSE;
                ...
                g_lpDs3dListener->SetRolloffFactor(.01,DS3D_DEFERRED);
...
BOOL EnterVideoModeWHBD(int width, int height, int bitdepth)
{
    ...
    g_lpD3DRMDevice->SetDither(TRUE);
    ...
    g_lpD3DRMViewport->SetBack(D3DVAL(150.0f));
...
/*	--------------------------------------------------------------------------------
	...
	Info 		:
*/
BOOL DoSplashScreen(char *fname)
{
	...
	sprintf(temp,"%sgraphics\\%s",rootPath,fname);
    fp = fopen(temp,"rb");
    if (!fp) goto fail;
...
//    fread(&dummy, 4, 1, fp);
	...
	fread((char *)&bmp_header,1,sizeof(BMPHeaderType),fp);			// read header
    ...
    for (i = 0; i < 256; i ++)
    {
        ppe[i].peBlue   = getc(fp);
        ppe[i].peGreen = getc(fp);
        ppe[i].peRed  = getc(fp);
        ppe[i].peFlags = PC_NOCOLLAPSE;
		getc(fp);
    }

	pSurf += ddsd.lPitch*479;
    for (i = 480; i > 0; i--)
    {
 ...
 */
HRESULT D3DAppICreateSurface(LPDDSURFACEDESC lpDDSurfDesc,LPDIRECTDRAWSURFACE FAR *lpDDSurface)
{
    HRESULT result;
//    if (d3dappi.bOnlySystemMemory)
//        lpDDSurfDesc->ddsCaps.dwCaps |= DDSCAPS_SYSTEMMEMORY;
    result = g_lpDD->CreateSurface(lpDDSurfDesc,lpDDSurface,NULL);
    return result;
}
 ...
 * GetSurfaceDesc call.
 */
HRESULT D3DAppIGetSurfDesc(LPDDSURFACEDESC lpDDSurfDesc,LPDIRECTDRAWSURFACE lpDDSurf)
{
    HRESULT result;
