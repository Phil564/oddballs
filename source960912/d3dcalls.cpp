...
{
    LastError = d3dappi.lpDD->QueryInterface(IID_IDirect3D,(LPVOID*)&d3dappi.lpD3D);
    if (LastError != DD_OK) {
...
D3DAppIEnumTextureFormats(void)
{
    ...
    LastError =
         d3dappi.lpD3DDevice->EnumTextureFormats(EnumTextureFormatsCallback,(LPVOID)&StartFormat);
    if (LastError != DD_OK) {
...
D3DAppICreateDevice(int driver)
{
    ...
    LastError = d3dappi.lpBackBuffer->QueryInterface(d3dappi.Driver[driver].Guid,
...
D3DAppISetRenderState()
{
    ...
    LastError =
        d3dappi.lpD3DDevice->CreateExecuteBuffer(&debDesc, &lpD3DExCmdBuf, NULL);
    if (LastError != D3D_OK) {
    ...
    LastError = d3dappi.lpD3DDevice->BeginScene();
    if (LastError != D3D_OK) {
        D3DAppISetErrorString("BeginScene failed in SetRenderState.\n%s",
                              D3DAppErrorToString(LastError));
        goto exit_with_error;
    }
    LastError = d3dappi.lpD3DDevice->Execute(lpD3DExCmdBuf,d3dappi.lpD3DViewport,D3DEXECUTE_UNCLIPPED);
    if (LastError != D3D_OK) {
        D3DAppISetErrorString("Execute failed in SetRenderState.\n%s",
                              D3DAppErrorToString(LastError));
        goto exit_with_error;
    }
    LastError = d3dappi.lpD3DDevice->EndScene();
    if (LastError != D3D_OK) {
