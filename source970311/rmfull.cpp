        ...
        int nCmdShow)
{
	...
	{
		OpenCDDevice(myglobs.hWndMain);
		playCDTrack(myglobs.hWndMain, 1);
	}
...
//wooden
    ...
    myglobs.hWndMain = CreateWindowEx(
         ...
         START_WIN_SIZE, 250,//START_WIN_SIZE,
         NULL,                              /* parent window */
         NULL,                              /* menu handle */
...
//wooden
    ...
    if (!BuildScene(myglobs.dev,myglobs.view,myglobs.scene,camera.frame))
        return FALSE;
...
CreateD3DRM(HWND win)
{
    ...
    rval = lpD3DRM->CreateFrame(myglobs.scene,&(camera.frame));
    if (rval != D3DRM_OK) {
        Msg("Failed to create the camera's frame.\n%s", D3DAppErrorToString(rval));
        return FALSE;
    }
    rval = camera.frame->SetPosition(myglobs.scene, D3DVAL(0.0), D3DVAL(0.0), D3DVAL(0.0));
    if (rval != D3DRM_OK) {
...
//try and get full screen 320 x 240 x 16

	
    if(((detailFlags & DF_HARDWARE) == 0) && ((!D3DAppChangeDriver(0, NULL))))
	{
        ReportD3DAppError();
        CleanUpAndPostQuit();
    }
	...
	if(detailFlags & DF_FULLSCREEN)
	{
		...
		if(detailFlags & DF_WOODY)
			if (!D3DAppFullscreen(1)) 
			{
				ReportD3DAppError();
				CleanUpAndPostQuit();
			}
...
AfterDeviceCreated(int w, int h, LPDIRECT3DVIEWPORT* lplpViewport, LPVOID lpContext)
{
    ...
    rval = lpD3DRM->CreateViewport(myglobs.dev,camera.frame,0,0,w,h,&myglobs.view);
    if (rval != D3DRM_OK) {
    ...
    rval = myglobs.view->SetBack(D3DVAL(300.0));
    if (rval != D3DRM_OK) {
...
//restore user surfaces
	for(x = 0; x < MAX_USER_SURFACES; x++)
	{
		surface = &userSurface[x];

		if((surface->active) && (surface->display) && (surface->buffer))
		{
			if (surface->buffer->IsLost() == DDERR_SURFACELOST) 
			{
...
/*				
				...
				d3drval = surface->buffer->Restore();
				if (d3drval != DD_OK) 
				{
					dprintf("restore Surface: Restore failed [%s]\n", D3DAppErrorToString(d3drval));
					return FALSE;
				}
				gfxLoadBMPIntoSurface(surface->filename, surface->buffer, 0, 0);
*/
			}	
		}
	}
...
//				case VK_TAB:		controlMethod = 1 - controlMethod;
//									break;
	        }
			break;
...
InitGlobals(void)
{
	...
	if(detailFlags & DF_GOURAUD)
	    myglobs.RenderQuality = D3DRMLIGHT_ON | D3DRMFILL_SOLID | D3DRMSHADE_GOURAUD;
	else
	    myglobs.RenderQuality = D3DRMLIGHT_ON | D3DRMFILL_SOLID | D3DRMSHADE_FLAT;
